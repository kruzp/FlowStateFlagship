...
/**
 * FlowHero wires the Flow Material engine (Renderer / Simulation /
 * InputController / ForceInjector) into a bounded page container instead
 * of the fullscreen body the standalone engine demo used. It is an
 * orchestration adapter only — it does not modify or extend any engine
 * internals (core/, interaction/, render/Renderer.ts, or the display
 * shader are used exactly as shipped).
 *
 * Production concerns layered on top, outside the engine itself:
 *  - sizes/observes a container element rather than the window
 *  - pauses the render loop when the tab is hidden
 *  - respects prefers-reduced-motion by damping ambient drift
 *  - lowers grid resolution on small/low-power viewports
 */
export class FlowHero {
  ...
  constructor(container: HTMLElement) {
    ...
    if (!this.isReady) {
      // Caller (main.ts) is responsible for showing a fallback state.
      ...
      this.displayMaterial = null as unknown as THREE.ShaderMaterial;
    ...
    this.simulation = new Simulation(webgl, Math.max(width, 1), Math.max(height, 1), configOverrides);
    ...
    this.displayMaterial = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uTexelSize: { value: this.simulation.dyeTexelSizeVec },
      },
    });
  ...
  private startLoop(): void {
    if (this.frameId !== null || !this.isReady) return;
    this.clock.getDelta(); // discard elapsed time while paused/hidden
    const loop = (): void => {
