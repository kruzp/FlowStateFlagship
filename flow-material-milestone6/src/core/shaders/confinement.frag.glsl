...
void main() {
  ...
  float centerRaw = texture2D(uVorticity, vUv).x;

  // Use a spatially-averaged vorticity, not the raw single-texel value, as
  // the force magnitude. Confinement differentiates an already-discrete
  // field a second time (gradient of vorticity, itself a derivative) —
  // without this averaging, any single-texel noise gets amplified into
  // visible grid-scale blocky artifacts instead of the intended large
  // swirls. This softens exactly that, without weakening real structure.
  float center = (centerRaw * 2.0 + left + right + bottom + top) / 6.0;
